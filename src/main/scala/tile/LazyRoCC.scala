...
trait HasLazyRoCCModule extends CanHavePTWModule
  ...
  this: RocketTileModuleImp with HasFpuOpt =>
...
class CharacterCountExampleModuleImp(outer: CharacterCountExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val recv_beat = RegInit(UInt(log2Up(cacheDataBeats + 1).W), init = 0.U)

  val data_bytes = VecInit.tabulate(cacheDataBits / 8) { i => recv_data(8 * (i + 1) - 1, 8 * i) }
  ...
  val chars_found = PopCount(needle_match.zipWithIndex.map {
    case (matches, i) =>
      // val idx = Cat(recv_beat - 1.U, UInt(i, beatOffset))
...
class DandelionMemoryExampleModuleImp(outer: DandelionMemoryExample)(implicit p: Parameters) extends LazyRoCCModuleImp(outer)
  ...
  val accel = Module(new test05DF)

  when(io.mem.req.fire){
    isWrite := accel.io.MemReq.bits.iswrite
  }

  //Make memory req
  // MEMORY REQUEST INTERFACE
  accel.io.MemReq.ready := io.mem.req.ready
  io.mem.req.valid := accel.io.MemReq.valid
  io.mem.req.bits.addr := accel.io.MemReq.bits.addr
  io.mem.req.bits.tag := accel.io.MemReq.bits.tag
  io.mem.req.bits.cmd := Mux(accel.io.MemReq.bits.iswrite, M_XWR, M_XRD) // perform a load (M_XWR for stores)
  //TODO: needs to be what is right size?
  io.mem.req.bits.size := log2Ceil(8).U
  io.mem.req.bits.signed := false.B
  io.mem.req.bits.data := accel.io.MemReq.bits.data // we're not performing any stores...
  io.mem.req.bits.phys := false.B

  accel.io.MemResp.valid := io.mem.resp.valid
  accel.io.MemResp.bits.data := io.mem.resp.bits.data
  accel.io.MemResp.bits.tag := io.mem.resp.bits.tag
  accel.io.MemResp.bits.tile := 0.U
  accel.io.MemResp.bits.iswrite := isWrite
  accel.io.MemResp.bits.valid := true.B
